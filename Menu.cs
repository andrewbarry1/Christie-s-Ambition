...
namespace TheGame
{
	public class Menu
	{
		...
		public Menu (Vector2 loc, string[] options, menuAction[] a, int r, int c, string font, ContentManager Content)
		{
			...
			size = new Vector2(textSystems[farthestRightOption].location.X + textSystems[farthestRightOption].getWidth() - location.X - 12,
			                   textSystems[textSystems.Length - 1].location.Y + textSystems[textSystems.Length - 1].getHeight() - location.Y - 12);
		...
		public void handleInput(InputHandler inputHandler) {
			...
			if (inputHandler.allowSinglePress (Keys.Back)) {
				inUse = false;
				cursor = 0;
			}
		...
		public void draw(SpriteBatch spriteBatch) {
			...
			spriteBatch.Draw (yellowTexture, textSystems[cursor].location, null, 
			                  Color.White * 0.25f, 0f, Vector2.Zero, new Vector2(textSystems[cursor].getWidth() - 16, textSystems[cursor].getHeight() - 16),
